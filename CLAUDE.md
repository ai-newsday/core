...
## 汇报方式（`.claude/skills/manager-report`）
每个工作单元结束按五段汇报：做了什么 / 状态 / 产物 / 证据强度 / **唯一下一个决策点**，然后停下等我。
- 每条有后果的陈述心里要能归到 **[已验证事实] / [提案] / [已决定]**；不得把提案写成事实，不得从沉默推断批准。
- **一次只问一个问题**；其余决策点写进 KANBAN 待决策区。
- "测试全绿" ≠ "通过"；没验证过真实效果就报**证据不足**。
- 提方案前先确认它依赖的信号真的存在（本仓库四次实测里三次推翻了动手前的方案）。
