...
## 汇报与证据
- 角色、汇报结构、决策边界用 **`manager-harness`** skill（工作单元结束报四段：能做什么 / 怎么验 / 还缺什么 / 唯一下一步，然后停下等我）。
- 输出形状用 **`i-have-adhd`**：首行就是可执行动作，多步编号，一次一个决策点，不写开场白和结尾寒暄。
- 本仓库特有的证据纪律在 **`.claude/skills/newsday-evidence`**：提方案前先量信号、"测试全绿"≠"生效了"、纯函数必须配接线测试。
