...
from src.core.types import (NewsItem, SourceType, ScoringConfig, RunContext)
from src.pipeline.score import compute_scores

NOW = datetime(2026, 5, 30, 12, tzinfo=timezone.utc)


def _it(link, source="src", st=SourceType.PAPER, signals=None):
    return NewsItem(title_en="X", link=link, source=source, source_type=st,
                    published_at=NOW, cluster_id="c1", related_links=[],
                    signals=signals or {})


def _ctx():
    return RunContext(run_id="g", now=NOW,
                      logger=logging.getLogger("golden-score-pop"))
...
def test_popularity_weight_lifts_visibility():
    items = [_it("low", signals={"upvotes": 0}),
             _it("hi",  signals={"upvotes": 100})]
...
def test_popularity_sums_multiple_signal_keys():
    ...
    cfg = ScoringConfig(popularity_weights={
        "upvotes": 0.5, "hn_points": 1.0, "likes": 0.25})
    scored = compute_scores(items, {}, cfg, _ctx())
...
def test_popularity_cap_caps_at_max():
    ...
    cfg = ScoringConfig(popularity_weights={"hn_points": 1.0},
                        popularity_cap=15.0)
    scored = compute_scores(items, {}, cfg, _ctx())
...
def test_popularity_handles_non_numeric_gracefully():
    items = [_it("a", signals={"upvotes": "not-a-number"}),     # 脏数据
             _it("b", signals={"upvotes": None}),
             _it("c", signals={"upvotes": -5})]                  # 负数
    cfg = ScoringConfig(popularity_weights={"upvotes": 1.0})
    scored = compute_scores(items, {}, cfg, _ctx())
    by_link = {s.link: s for s in scored}
    # 非数 / None / 负数 → 0 (不抛, 不污染)
