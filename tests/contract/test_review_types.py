...
def test_reviewed_item_extends_interpreted_item():
    it = _interp()
    r = ReviewedItem(**it.model_dump(), review_action="keep", was_edited=False, edited_fields=[])
    # 继承上游不变量
...
def test_reviewed_item_edited_fields_recorded():
    it = _interp()
    r = ReviewedItem(
        **it.model_dump(), review_action="edit", was_edited=True, edited_fields=["title", "summary"]
    )
