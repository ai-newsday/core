...
def test_flip_draft_cli_rewrites_file(tmp_path):
    ...
    p.write_text("---\ndraft: true\n---\nbody\n", encoding="utf-8")
    r = subprocess.run(
        [sys.executable, "-m", "src.tools.flip_draft", str(p)], capture_output=True, text=True
    )
...
def test_flip_draft_cli_missing_file_is_noop(tmp_path):
    ...
    r = subprocess.run(
        [sys.executable, "-m", "src.tools.flip_draft", str(missing)], capture_output=True, text=True
    )
