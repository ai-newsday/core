...
| 取动作 | 只有该 link 有显式决策（`decisions[item.link]` 存在）才产事件，`action = decisions[item.link].action`；无决策 = 沉默 = 中性（不产事件），显式 `drop` 仍产 `drop` 事件 |
...
| 2 | 派生范围：只为有显式决策的条目派生事件（显式 `drop` 仍产 `drop` 事件；无决策条目沉默，不产事件） |
...
| 12 | 只算不接：本层不修改任何上游条目的 `score`/`genre`/…（不碰打分） |

## 9. golden 用例（≥6，冻结输入 fixtures）

| # | 用例 | 断言要点 | 关联不变量 |
|---|---|---|---|
| 1 | 派生含 drop：3 条，1 条决策 drop、1 条 edit、1 条无决策 | 产 2 事件（drop/edit）；无决策条目不产事件 | 2 |
