...
| 分类速览 | 按 `source_type` 分组，组间按既定类型顺序，组内保上游序 |
...
class Overview(BaseModel):
    type_distribution: dict[str, int]   # {"paper": 2, "model": 1, ...} 按类型计数
    keywords: list[str]                 # 高频关键词(聚合 tags), 取 Top N

class CategorySection(BaseModel):
    source_type: str                    # paper | model | tool | official | community | news | blog
...
| `sum(overview.type_distribution.values()) == item_count` | 类型分布计数等于总条数 |
...
| 规则 | 说明 |
|---|---|
| 分组键 | `item.source_type`（值如 `paper`/`model`/…） |
| 组间顺序 | 按 `config.type_labels` 声明的类型顺序（缺省顺序见 §6.1）；不在表里的类型排末尾 |
| 组内顺序 | 保上游序（不再二次排序） |
| 空类目 | 没有条目的类型不产 `CategorySection` |
| 标签 | `label` 取 `config.type_labels[source_type]`，缺则回退原始英文 |
...
| 字段 | 怎么算 |
|---|---|
| `type_distribution` | 按 `source_type` 计数，键按 §6.1 类型顺序排列（确定性） |
...
type_labels:                             # source_type → 中文展示名(也定义组间顺序)
  official: "官方"
  paper: "论文"
  model: "模型"
  tool: "工具 / 开源"
...
`PublishConfig`（dataclass，默认值同上）：`must_read_count` / `top_keywords` / `pending_watermark` / `type_labels`。`type_labels` 的**键顺序**即分类速览的组间顺序与数据概览的计数顺序。
...
| 3 | 必读子集：`must_read` 的每条都出现在其 `source_type` 对应的 `CategorySection` 里 |
| 4 | 类型分布守恒：`sum(type_distribution.values()) == item_count` |
| 5 | 组间顺序：`categories` 按 `config.type_labels` 键序，非空类目才出现 |
