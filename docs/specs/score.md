...
class ScoreResult:
    ...
    quota_report: dict[str, QuotaLine]   # key = source_type
...
| 维度 | v1 计算来源 | 备注 |
|---|---|---|
| `机构影响力` | `dimension_scores[source_type].机构影响力` + `priority_bonus[priority]` | priority 折进此维度（权威性），不另立 breakdown 键 |
| `一手性` | `dimension_scores[source_type].一手性` | paper/official/model 高，news/community/blog 低 |
| `技术价值` | `dimension_scores[source_type].技术价值` | |
| `产业影响` | `dimension_scores[source_type].产业影响` | |
| `扩散潜力` | `dimension_scores[source_type].扩散潜力` | |
...
1. 按 `source_type` 分组；组内按 `score` 降序，**同分用 `published_at` 升序再用 `link` 升序兜底**（确定性 tie-break）。
2. 每组取前 `quota[source_type]` 条（`quota` 缺该类型视为 0）：`selected[type] = 该组前 min(quota[type], available[type]) 条`。
3. `selected_items` = 各类型入选并集，按 `score` 降序（同分同 §5.4.1 tie-break）排列。
4. **严格按类型，不跨类型补位**（v1 保持简单；余量跨类型 fill 留 v2）。`total_limit` 作为总上限的健全性校验：`sum(quota.values()) <= total_limit` 由 config 保证；若入选总数超过 `total_limit`（理论上不会，因各 quota 之和受限），截断最低分。
5. `quota_report[type] = QuotaLine(type, available, quota, selected)`，覆盖出现过的全部类型。

## 6. 配置：`config/scoring.yaml`

```yaml
# 维度基分: source_type → 各维度贡献(PRD §5.5 维度键, 0-100 多维). 全部可调, 不写死.
dimension_scores:
...
# 类型配额(PRD §4.2). 各 quota 之和应 <= total_limit.
...
对应 `ScoringConfig`（dataclass，默认值与上表一致）字段：`dimension_scores: dict[str, dict[str, float]]`、`priority_bonus: dict[int, int]`、`priority_bonus_default: int`、`recency`（拍平为 `fresh_hours/fresh_bonus/mid_hours/mid_bonus/stale_hours/stale_penalty`）、`same_source_penalty: float`、`quota: dict[str, int]`、`total_limit: int`、`sources_registry_path: str`。
...
| 某 `source_type` 0 条 | 该类型 `selected=0`，不编造（PRD「宁可少写不可编造」） |
| 某 `source_type` 不在 `dimension_scores` | 该类型各维度取 0（enum 封闭，正常不发生；防御性） |
...
> 测试用**冻结的 NewsItem fixtures**（含 source_type / source / published_at），注入固定 `ctx.now` 与固定 priority map，使打分与入选确定、可断言，不依赖网络与真实时间。
...
- 每条打分后 emit `item_scored{link, source_type, score}`（量大可仅 debug 级或聚合，复用 dedup 的 `emit`）。
- 配额应用后每类型 emit `quota_applied{source_type, available, quota, selected}`。
- 每条入选 emit `item_selected{link, source_type, score}`（PRD §6.2 埋点）。
