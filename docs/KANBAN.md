...
| ✓ | 优先 | 任务 | 详情 |
|---|---|---|---|
| ☐ | **A组** | **时区从北京改为 UK(用户所在地)** | 2026-08-01。`src/cli.py:529` `_beijing_report_date()` 硬编码 `Asia/Shanghai`;`collect.yml`(cron `0 0,5,12`)/`finalize.yml`(cron `0 1`)注释均按北京时间。需改成 UK(注意 BST/GMT 夏令时切换,`Europe/London` 而非固定 UTC 偏移)。**待问用户**:是长期在 UK 还是临时(决定硬编码 `Europe/London` 还是做成 config 可配)。A 组四条强耦合(日期标签、提醒时间都依赖时区),必须一起改。 |
| ☐ | **A组** | **草稿日期改成当天(而非前一天)** | 2026-08-01 用户提出。现 `run_tick` finalize 分支 `date_label = (now.date() - timedelta(days=1))`(晨报总结昨天,M2 #33 的有意设计)。**⚠️ 与下方 P0「finalize 混入超出目标日期条目」直接冲突**:那条的既定修法是"published_at 不晚于 date_label 当天 23:59",若 date_label 改成当天,该上界含义完全改变(变成"不晚于今天结束"=几乎不过滤)。**两条必须一起重新设计,不能分开做**。 |
| ☐ | **A组** | **零决策兜底改为不自动发** | 2026-08-01 用户明确:"我没做出反应的最好不要结算"。现 `src/pipeline/tick.py::run_finalize_tick` 有 `if decisions: 走确认门 else: report_items = list(interpreted_items)` 的零决策兜底(一条未点 → 全量进 publish 由地板/配额截 top-N)。改成零决策时也不自动发。**注意副作用**:用户漏审一天 = 当天无报,需确认这是想要的(还是要留个"至少发 N 条"的保底)。 |
| ☐ | **A组** | **每晚 22:00(UK)前提醒用户做筛选** | 2026-08-01 用户提出,**新功能**:现有 notifier 只在 collect 时推卡片、finalize 时推终稿,没有"待审提醒"这类主动 nudge。需新增一个定时任务,在当地 22:00 前检查当日未决策条目数,有则推一条汇总提醒。**待问用户**:提醒里要不要带未审条目列表/直达链接,还是只报个数。 |
...
| ☐ | **A组** | **finalize 报告混入超出目标日期的条目(逻辑漏洞)** | 2026-07-29 用户发现"7-28 日报里有 7-29 的新条目"。**已诊断,真 bug**:`content/posts/2026-07-28.md` 由 2026-07-29 01:00 UTC(北京 09:00)那次 finalize 生成,"晨报总结昨天"的 `date_label = 今天-1天` 设计本身没错;**真正的漏洞是 `collect()` 的时间窗口(`window_hours`/`max_window_hours`)只限制"不能太旧",完全没有"不能比 date_label 覆盖的那天更新"的上界**——finalize 在北京早上跑,能抓到当天已发布的最新内容,这些内容会被塞进标着"昨天"的报告里。原修法:给 finalize tick 的 collect 加一道"published_at 不晚于 date_label 当天 23:59"的上界过滤。**⚠️ 2026-08-01 该修法已失效**:用户要求草稿日期改成当天(见上 A组),date_label 语义一变,这个上界就几乎不过滤任何东西。**本条必须跟 A组「草稿日期改当天」「时区改 UK」一起重新设计**,不能单独做。归 **A组**。 |
