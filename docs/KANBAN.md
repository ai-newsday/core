...
| ☐ | **P0** | **release_importance 模型链全死,fail-open 从未真正判定过** | 2026-08-11 跑配额 dry-run 时意外实锤:`config/enrich.yaml` 的 `release_importance` 链——主模型 `deepseek-ai/DeepSeek-V4-Flash`、备用 `moonshotai/Kimi-K2.6`——**拿真实请求逐个测过,ModelScope 上都已下线**("has no provider supported"),再备用 `agnes` 又没配 `AGNES_API_KEY`。三个全挂 → 每条 GitHub Release 无差别 fail-open 到 tier=2,`release_importance` 这段时间根本没在判定,不是判定标准松。**大概率是下面"GitHub Releases 低版本没筛掉"(openhands 一次出 4 版本)那条投诉的真根因**。顺手把整个 14 个模型的旧注册表测了一遍,8 个已死,详见 memory `modelscope-model-registry`;活着的:`DeepSeek-V4-Pro`/`Ring-2.6-1T`/`Qwen3.5-397B-A17B`/`Step-3.7-Flash`/`Intern-S1-Pro`/`Intern-S2-Preview`。修法明确:把 `release_importance` 的 `models`/`fallback_models` 换成验证过存活的模型,范围小,可以直接排期。**修完后需要重新跑一次配额 dry-run**——下面「草稿池/终版配额哲学」的数字是在这个 bug 存在的情况下测的,`announcement` 64 条候选里有多少是"没在筛"的 GitHub Releases 撑起来的还不知道,基线可能会变。 |
