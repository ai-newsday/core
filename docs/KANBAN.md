...
| ☐ | **P0(D组升级)** | **TG 大量条目未解读(extractive_fallback)——2026-08-27 拿到真实诊断,可能是"推理模型吃光 max_tokens"** | 2026-08-01 用户反馈"今天 TG 还有很多没解读的"。**下一步是诊断不是修**的诊断今天做了:验证 source 泄漏 P0 修复时,拿真实还原的案例跑了一遍生产 `interpret.yaml` 完整模型链(7 个模型),**全部依次失败**,兜底到 `extractive_fallback`。查证:①4 个模型(`DeepSeek-V4-Flash`/`Kimi-K2.6`/`Kimi-K2.5`/`Ling-2.6-1T`)是 8-11 已查实的死模型,还没从链上清掉;②过去撑住这条链的两个主力 `DeepSeek-V4-Pro`/`Ring-2.6-1T` 这次 `choices: null`,`Qwen/Qwen3.5-397B-A17B` 超时。用 curl 单独排查发现更普遍的机制问题:这几个新模型是**推理模型**,会先输出一大段看不见的 `reasoning_content`(思维链),`max_tokens` 预算被推理吃掉后真正的 `content` 要么截断成空、要么响应结构整体异常(`choices` 为 null)。`config/interpret.yaml` 的 `max_tokens` 其实已经从 800 调到 1500(现存注释"治 DeepSeek verbose 截断"),说明这个坑踩过一次,但今天这条链仍然全灭,**样本量还小(一次真实调用),需要更多次真实 tick 数据确认是持续性问题还是那次巧遇**。下一步:①先清掉链上已知的 4 个死模型;②跑多次真实 `interpret` tick,统计 `interpret_error` 的 `error_type` 分布,确认"推理模型截断"是不是主因;③如果确认,可能需要给推理模型单独设更大的 `max_tokens` 或探索关闭 thinking 的请求参数(如果 ModelScope 支持)。 |
...
| ☐ | **P1** | **GitHub Releases 时效性差:几天前的 release 当"今天"发** | 2026-08-27 用户反馈。**已用代码验证根因**(不是猜测):`src/core/types.py:83` `CollectionConfig.window_hours` 默认 **72 小时**,是全局统一窗口,当初(见该行注释)是为了不漏抓 paper/tool/blog 这类慢更新源才从 24h 拉宽的。但 `github_releases` 读者预期是"今天/昨天发生的事",跟 paper/blog"慢更新可以容忍几天"是两种完全不同的时效性预期,被同一个 72h 全局窗口削平——2-3 天前的 release 依然能当"新"候选采集、审阅、发布。修法方向:`window_hours` 做 per-genre 或 per-adapter 覆盖(github_releases 收紧到 24-48h,paper/blog 保留 72h),不能简单全局调小(会重新引入"慢更新源被漏抓"的原问题)。 |
