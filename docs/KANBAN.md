...
**当前状态、这一轮做完的事、等你决定的事,都在 `docs/MAINLINE.md` 顶部"现在的主线"一节**,这里不重复。

**等你决定(详情见 MAINLINE)**
1. 要不要先查清 agnes 的真实限额(推荐要;只记日志,不改行为)。
2. x:pixai_official 这类"保留率高但几乎全靠跳过"的来源砍不砍。

**刚合并**:#206 一期一模型(agnes 优先)。

> 本表 §2 之后的内容大多写于 09-05 以前,长段落是当时的工作记录。**[Agent 提案]** 把这些压缩成每条一行、
> 细节只留 PR 链接;要不要做由你定,未经同意不删。
...
| 项 | 卡在哪 |
|---|---|
| **#160 日期命名撞车**(命名已修 #165, 剩一半) | 两次 finalize 落在同一个伦敦日 → 同一个 `date_label` → 跨天去重被"同 label 视为重跑"的豁免整个架空, K2-Horizon-MoVA 与 Runway GWM Worlds 2 **链接完全相同**却连发两天, 且第二次直接覆盖了已发布的 `posts/2026-09-04.md`。用户要的命名规则:**文件名用发布当天, 内容是前一天的**。两条路等确认——**A** 保持 23:00 跑、label 记成第二天(改动小, 但"运行日≠标签日"永久错开); **B** 把 cron 挪到本地午夜之后(label 自然等于发布日, 无偏移逻辑, 多两小时审阅时间, 代价是草稿晚约 20 分钟)。用户 2026-09-05 选 A, 理由是**伦敦午夜后发布 = 北京早上八点**, 刊物是给中国读者的早报。**已修(#165)**: 晚上跑的标第二天, 午夜后跑的标当天(那正是它要发的那份, 重跑必须落回同一个 label)。两次真实运行的时间戳直接当回归用例。**剩下的一半**: 同一份刊物重跑时, 仍会重新放行该刊早前版本已经发出去的条目——label 正确之后不会再自动撞车, 但手动重跑一份已发布的刊物仍会重复。**这不是 #147 的语义去重缺口**——现有按链接去重本来就能拦住。 |
| **#147 跨天去重只认相同链接**(待决策) | Qwen-Drive 连发两天:9-03 是 arXiv 论文页,9-04 是同一项目的 X 发布帖。`tick.py` 用 `sha256(link)` 比对,两个不同 URL 就是两个不同 id。**要用户先定"同一项目"还是"同一件事"算重复**——9-04 那条是 4B 新版本发布,严格说是新事件,不是重复报道。定了才好在三条路子里选(向量比对 / 实体名 key / 复用 storylink 的 LLM 判断)。另有一个必须先量的问题:向量比的是"arXiv 摘要"对"一条推文",相似度够不够高又不误杀,没测过。 |
| **#121 content_certain 命中率**(阻塞于依赖) | 需要一次 LLM 全绿的真实 dry-run 才能量。9-03/9-04 两晚 ModelScope 与 agnes 轮流故障(agnes 500、ModelScope 400/超时),没跑成。 |

> **向量库现状值得单独记一笔**(#147 查出来的):`VectorStore` 协议只有 `upsert` 没有 `search`,`dedup()` 只写不查,实现是进程一退就没的 `InMemoryVectorStore`。CLAUDE.md 里写的 Qdrant **哪儿都没接上**。跨天语义去重不是"坏了",是从来没建过。
...
> **2026-09-02/04 版式与配图连修四轮 + 两次"先量再改"推翻了初始方案**。用户开始每天定点发公众号,连续三天逐条 review 出问题,当天修当天发。四轮:①版式(H2/配图/X 保留配额/发卡池 100,#141) ②渲染硬伤(美元符号/套话/空 body/标题没接线,#136-#138) ③标题摘要变丰富+超长重试+诊断日志(#142/#146) ④配图三缺陷(#148/#150)。**两次实测推翻了我的初始方案**:配图按体积过滤(图标 101KB 比真插图 38KB 还大,会误杀真图)、抓 README 兜底(三个模型页里一个只有 shields.io 徽章,会把徽章当配图)——两次都是先量后改才没做错。**同期暴露的两个结构性问题记在 §2 和下表**:向量库从来没接上(#147)、`interpret()` 顺序循环导致墙钟时间随发卡池线性增长。

| ✓ | 优先 | 任务 | 详情 |
|---|---|---|---|
| ☑ | ~~P0~~ **已修 #164** | **配额把用户 keep 的条目静默砍掉** | 2026-09-04 实测: `review_done kept_count=10` → `report_built item_count=7`, **用户明确按了 keep 的 3 条被 genre 配额丢掉**。发出来的正好是 paper:3 / model:1 / announcement:3, 而配额上限是 paper:3 / model:3 / announcement:3——两类顶格。当天最大的新闻「NVIDIA 129 亿美元收购 Hugging Face」只出现在被拒的标题里、没进正文, 极可能就是被砍的三条之一。配额是用来在大池子里平衡结构的, **不该用来砍一个已经人工筛过的 10 条集合**。 |
| ☑ | ~~P0~~ **已修 #163** | **标题连挂三晚: 3 个事件塞不进 64 字** | 2026-09-02/03/04 连续回退成朴素标题。#142 的诊断日志这次说清了原因, 而原因正是 #142 自己: spec §2 原本算过账、定的是"默认 2 个事件"(`【AI日报】` 占 6 字、每个 ` \| ` 占 3 字, 只剩约 52 字, 而单个模型名可达 13 字), 我按用户要求改成目标 3 个之后**稳定超长**。实测两次 75 字与 88 字, 重试比第一次还长。**回退成朴素标题比 2 个好事件更差**——这是我改坏的, 要退回 2 个并补一条把算术钉死的测试。 |
| ☑ | ~~P0~~ **已修 #153** | **`interpret()` 顺序循环 → 并发** 2026-09-04 合并。`concurrency` 默认 4 走配置。真实 provider 实测 8 条:524.5s → 238.8s,**2.2x**(不是 4x:两波调度下每波只能跟最慢那条一样快,单条延迟均值 65s 且波动大)。40-60 分钟按此落到 18-27 分钟,好转但不算好;下一个杠杆是调高并发,要看生产 429 再定。顺序一致性由测试锁死(下游配额/参考编号依赖它)。原始记录如下: |
| ☐ | ~~P0~~ | ~~`interpret()` 是顺序循环,发卡池翻倍=耗时翻倍~~ | 2026-09-03/04 连续两晚 finalize 跑 40-60 分钟,用户直接问"这他妈卡在哪里了"。根因两条叠加:(a) 我把 `card_pool_limit` 50→100 时**只算了 LLM 调用次数成本,没算墙钟时间**——`interpret()` 一条一条等 LLM 返回,100 条就是 100 次串行往返;(b) 同期 ModelScope/agnes 本身不稳(400/500/超时),每条重试更久。**修法不是把发卡池缩回去**(那会把刚救回来的 X 覆盖率搭进去),是让 interpret 并发跑:同样的调用次数与成本,墙钟从"N × 单条延迟"降到"N/并发 × 单条延迟"。未做未验证。 |
| ☐ | **P1(阻塞发布决策)** | **公众号外链图能否显示,至今未验证(#124)** | 见下方原条目。配图现在质量达标了(#148/#150),但如果公众号根本不渲染外链图,这些工作在公众号侧等于零——**这一条的优先级因此上升**,应当在下一次发布时直接拿真实文章验证。 |
| ☑ | ~~P1~~ **已修 #155/#156** | **公众号版流水线化** 2026-09-04 晚。`render_wechat()` 直接产出 `content/wechat/<date>.md`,`clean_wechat_tags()` 兜掉微信的标签截断,spec 2026-08-31 全部实现完毕。与网站版共用 `_render_items` 而不是写第二个渲染器——两个渲染器必然漂移,漂移正是手工那步出错的根源。过滤回退条目时重建报告而非渲染时跳过,保证目录/角标/参考表数的是同一批条目(分头过滤会在剔掉一条后三处错位,有测试专盯)。实测:真实报告 8 条三处编号一致;75 个真实标签改动 4 个(全是微信本会截断的),71 个未动。原始记录如下: |
| ☐ | ~~P1~~ | ~~目录仍是手工加的,公众号版整份都是手工转换~~ | 2026-09-03/04。目录规则已写进 spec(#143 §3b),但流水线只渲染网站版,公众号那份 md 每天由我手工转换——今天漏目录、之前漏标签清洗、漏摘要补全,**出错的都是手工那一步,不是流水线**。#126 的公众号版式实现应当提前:让流水线直接产出 `content/wechat/<date>.md`,把这些环节收进代码+测试。 |
| ☐ | **P0** | ~~标题生成了却没送到渲染层 + 今日看点前缀重复(#130)~~ **已修 #136** | 2026-09-01。`daily_take_done` 报 `title_generated: true`,成品标题却仍是 `AI Daily · <date>`——#129 把 `wechat_title` 一路穿过类型,却漏改 `cli.py` 里 `review()` 的调用点,参数有默认值 `None` 于是静默用了默认;单测因为直接构造 `ReviewResult` 正好绕过这个缺口。同时 `render_markdown` 硬编码的 `> **今日看点**：` 与新摘要自带的 `今日亮点：` 重复。**修的时候要补一个走 `run_tick` 真实路径的测试**,不能再靠手搓 `ReviewResult`。 |
| ☐ | ~~P1~~ | ~~正文 `$` 破坏渲染 + 每条都以套话收尾(#131)~~ **已修 #137,9-04 产出复核 0 次** | 2026-09-01 用户逐条编辑时发现。`$5090`/`$6.9K`/`$4.4K` 三个美元符号被 Markdown 当成行内数学公式,把文字拆成 `**性** **能** **逼** **近**`——AI 新闻里价格/成本/融资很常见,会反复出现,倾向在 prompt 层要求写「6900 美元」而不是转义。另:7 条里 2 条以近乎相同的「对从业者而言,这意味着…」收尾,是 prompt 要求「落到对从业者意味着什么」被模型用套话敷衍,需保留意图但禁掉这个口头禅。**都是 prompt 层改动,必须真实 dry-run 验证**(#129 的长度规则就是先例:prompt 写「必须 ≤120 字」实测 145)。 |
...
| ✓ | 任务 | 详情 |
|---|---|---|
| ☑ | 报告按发布日命名(#160/#165) | 2026-09-05。用户定的规则: **文件名用发布当天, 内容是前一天的**——伦敦午夜后发布对应北京早上八点, 刊物是给中国读者的早报。晚上跑的标第二天; 午夜后跑的标当天(那正是它要发的那一份, 重跑必须落回同一个 label, 否则会被当成新的一天把昨晚已发的全部重新放行)。两次真实运行(`09-03T23:04Z` / `09-04T22:52Z`)原本撞在同一 label, 直接当回归用例。**顺带修了个隐患**: 这个测试文件原来拿 `datetime.now()` 比对, 结果取决于跑测试的时刻, 过了傍晚就会因为跟代码无关的原因变红。 |
| ☑ | 配额不再砍掉人工 keep 的条目(#132/#164) | 2026-09-05。配额之和恰好等于 total_limit(3+3+3+2+1=12), 所以**任何类型分布不均的一天**, per-genre 上限都必然先于总量生效。09-04 实测: keep 10 条只发出 7 条, 3 篇论文被静默丢掉, 当天最大的新闻(NVIDIA 收购 Hugging Face)进了标题却没进正文。改成**只在装不下时才施加配额**。有意的行为变更, 不是狭义 bugfix; publish spec 原本完全没写过这一步, 补成 §5.5b。 |
| ☑ | 标题退回 2 个事件(#161/#163) | 2026-09-05。连续三晚回退成朴素标题, #142 的诊断日志说清了原因, 而原因正是 #142 自己把目标从 2 改成 3。实测两次 75 字与 88 字, 重试比第一次还长。spec 原本算过这笔账。两条测试把**算术**钉死(不是文案), 以后改回 3 个会红在测试里而不是每晚红在生产。**#142 那条测试的注释当初就预警过这个失败模式, 我写完还是改了。** |
| ☑ | 公众号版流水线化 + 标签清洗(#155/#156) | 2026-09-04 晚。`render_wechat()` 直接产出 `content/wechat/<date>.md`, 与网站版共用 `_render_items`(两个渲染器必然漂移, 而漂移正是手工转换反复出错的根源)。过滤回退条目时重建报告而非渲染时跳过, 保证目录/角标/参考表数的是同一批条目。`clean_wechat_tags()` 兜掉微信"遇符号即截断": 75 个真实标签改动 4 个(全是微信本会截断的), 71 个未动。**2026-09-05 首次真实生效, 人不再手工转换。** |
| ☑ | interpret 并发(#152/#153) | 2026-09-04。`interpret()` 原是纯顺序循环, 发卡池 100 就是 100 次串行等待, finalize 连着两晚跑 40-60 分钟。`OpenAICompatLLM` 无可变状态(字段构造后只读、每次调用自建 client), 线程池直接套在现有同步链上即可。测试钉死两条: 结果顺序按输入而非完成顺序(下游配额/参考编号依赖它)、并发是真的(`threading.Barrier`, 顺序实现会**超时失败**而不是慢慢通过)。实测 8 条 524.5s→238.8s(2.2x); **生产实测 40-60 分钟 → 26 分钟**。 |
| ☑ | 定时 finalize 连挂三晚: 推送被拒(#158/#159) | 2026-09-04。09-01/02/03 的 `schedule` 运行全部失败, 草稿之所以还在是因为每晚有人手动补跑——**从外面看完全像是自动跑通了**。流水线本身每次都成功, 死在 `Commit draft` 的裸 push(`! [rejected] master -> master`): finalize 要跑几十分钟, 期间任何一次合并都会让直推被拒。裸 rebase 也不够——抢先的那次通常是同日手动补跑, 改的是**同一个** `content/posts/<date>.md`, 是真冲突; 生成物按"后跑的赢"用 `-X theirs`。在一对本地仓库里完整复现验证。 |
| ☑ | 配图三缺陷(#144/#148/#150) | 2026-09-04。**A** YouTube 的 Atom feed 把播放器放在 `media:content`, 真图在 `media:thumbnail`——只过滤类型会让 YouTube 条目彻底丢图; 顺带堵上"`image_url` 两个写入方只有一个校验"。**B** arXiv 首图抓成 14x14 图标, **按体积过滤的方案被实测推翻**(图标 101KB > 真插图 38KB), 改用标签自带尺寸。**C** HF 模型页 og:image 是渐变模板卡; 抓 README 兜底也被实测否掉(三个真实页面里一个只有 shields.io 徽章)。真实网络验证 8 条: 4 条有图且全部为真图, 之前 6 张只有 2 张站得住。 |
| ☑ | 标题选题信号真正生效(#145/#146) | 2026-09-04。prompt 写着"大机构优先、评分高优先", 但 `build_daily_prompt` 只喂标题一行, 模型看不到 `publisher`/`score`, 规则**根本执行不了**。两个信号本就在 `InterpretedItem` 上, 只是没往下传。现每行 `- [分数\|机构类型] 标题`。⚠️ 与之同批的"目标 3 个事件"是错的, 见 §3 #161。 |
| ☑ | 配图三个缺陷全修(#148/#150) | 2026-09-04 用户"图经常找的有问题"。**A**:YouTube 的 Atom feed 把**播放器**放在 `media:content`(`type: application/x-shockwave-flash`),真图在 `media:thumbnail`——只加类型过滤会让 YouTube 条目彻底丢图,两半都得做;顺带堵上"`image_url` 两个写入方只有一个校验"的洞。**B**:arXiv 首图抓成 14x14 仓库图标;**按体积过滤的方案被实测推翻**(图标 101KB 比真插图 `dart_fig2.png` 的 38KB 还大),改用标签里声明的尺寸(14 对 476)。**C**:HF 模型页 og:image 是渐变模板卡(把模型名当图片文字重复一遍,而模型名就在图正上方);抓 README 兜底也被实测否掉(三个真实模型页:一个有真图、一个没图、一个只有 shields.io 徽章,会把徽章当配图)。真实网络验证 8 条:4 条有图且 4 张全部为真图,之前是 6 张图只有 2 张站得住。 |
| ☑ | 标题选题规则真正生效(#146) | 2026-09-04。prompt 里写着"大机构优先、评分高优先",但 `build_daily_prompt` 只喂标题一行——模型看不到 `publisher` 和 `score`,规则**根本执行不了**,只能从标题文字里猜机构(`OpenAI 推出…` 猜得中,`K2-Horizon-MoVA` 完全没依据)。两个信号本来就在 `InterpretedItem` 上,没往下传。现每行 `- [分数\|机构类型] 标题`,选题顺序写死:lab/company → 同档内分数 → 三条分散机构/主题 → 数字反差,字数仍是硬约束。**未验证**模型是否真按规则走(当晚 agnes 500)。 |
| ☑ | 标题/摘要变丰富 + 超长重试 + 诊断日志(#142) | 2026-09-04 用户要求"标题可以提到3个item,摘要可以提到4-5个"。实测 3 事件目标下模型经常一次性写到 64 字上限两倍且不会自己按 prompt 退化,加代码层面重试一次。同时补上 `enforce_title` 的拒绝日志——9-02/9-03 两次生产回退完全无迹可查,只看得到 `title_generated: false`;9-04 这个日志立刻定位到真因是 `agnes 500` 而非超字数。 |
| ☑ | 公众号版式:目录决策撤销(#143) | 2026-09-04。08-31 spec 明写"不做目录"(理由:公众号锚点跳不动),用户 9-01 手动加回、9-03 直接点名缺失——锚点论证没错,漏算的是目录的价值不在跳转在**扫读**。spec 已改,§3b 定死格式(无链接编号列表,与正文/参考链接编号一致)。 |
| ☑ | H2 小标题 + 逐条配图 + X 保留配额恢复 + 发卡池 100(#141/#135/#140) | 2026-09-02。用户逐条 review 出的版式问题一次修完。**`apply_reserved_quota` 曾从代码里整个消失**,X 实测 2/50 恢复到 20/50;发卡池 50→100 后最低分只从 77 掉到 75。⚠️ 代价:`interpret()` 是**纯顺序循环**,发卡池翻倍 = 墙钟时间翻倍(9-03 那次跑了 40+ 分钟),这条当时只算了调用次数没算耗时,见 §3。 |
| ☑ | 正文渲染硬伤:美元符号 / 套话结尾 / 空 body / 标题没接线(#136/#137/#138) | 2026-09-01 用户手工编辑当期时逐条挑出。`$5090` 被当成行内公式定界符把正文撕掉;每条都以同一句套话收尾;空 body 条目照发;`wechat_title` 穿了一路类型却**四个调用点全都在用 `None` 默认值**(测试直接给 `ReviewResult` 赋值绕过了 `review()`,所以没测出来)。9-04 真实产出复核:美元符号 0 次、套话 0 次。 |
