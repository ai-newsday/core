...
| 项 | 卡在哪 |
|---|---|
| **#160 日期命名撞车**(待决策) | 两次 finalize 落在同一个伦敦日 → 同一个 `date_label` → 跨天去重被"同 label 视为重跑"的豁免整个架空, K2-Horizon-MoVA 与 Runway GWM Worlds 2 **链接完全相同**却连发两天, 且第二次直接覆盖了已发布的 `posts/2026-09-04.md`。用户要的命名规则:**文件名用发布当天, 内容是前一天的**。两条路等确认——**A** 保持 23:00 跑、label 记成第二天(改动小, 但"运行日≠标签日"永久错开); **B** 把 cron 挪到本地午夜之后(label 自然等于发布日, 无偏移逻辑, 多两小时审阅时间, 代价是草稿晚约 20 分钟)。我倾向 B。**这不是 #147 的语义去重缺口**——现有按链接去重本来就能拦住。 |
...
| ✓ | 优先 | 任务 | 详情 |
|---|---|---|---|
| ☐ | **P0** | **配额把用户 keep 的条目静默砍掉(#132 实锤)** | 2026-09-04 实测: `review_done kept_count=10` → `report_built item_count=7`, **用户明确按了 keep 的 3 条被 genre 配额丢掉**。发出来的正好是 paper:3 / model:1 / announcement:3, 而配额上限是 paper:3 / model:3 / announcement:3——两类顶格。当天最大的新闻「NVIDIA 129 亿美元收购 Hugging Face」只出现在被拒的标题里、没进正文, 极可能就是被砍的三条之一。配额是用来在大池子里平衡结构的, **不该用来砍一个已经人工筛过的 10 条集合**。 |
| ☐ | **P0** | **标题连挂三晚: 3 个事件塞不进 64 字(#161)** | 2026-09-02/03/04 连续回退成朴素标题。#142 的诊断日志这次说清了原因, 而原因正是 #142 自己: spec §2 原本算过账、定的是"默认 2 个事件"(`【AI日报】` 占 6 字、每个 ` \| ` 占 3 字, 只剩约 52 字, 而单个模型名可达 13 字), 我按用户要求改成目标 3 个之后**稳定超长**。实测两次 75 字与 88 字, 重试比第一次还长。**回退成朴素标题比 2 个好事件更差**——这是我改坏的, 要退回 2 个并补一条把算术钉死的测试。 |
