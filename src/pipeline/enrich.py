...
import asyncio
from src.core.types import RawItem, EnrichConfig, RunContext
...
async def _enrich_one(item: RawItem, client, sem: asyncio.Semaphore,
                      ctx: RunContext) -> None:
    async with sem:
        try:
            hits = await client.search_url(item.link)
        except Exception as e:
            emit(ctx.logger, "enrich_error", link=item.link,
...
async def enrich_with_hn(items: list[RawItem], client,
                         config: EnrichConfig,
                         ctx: RunContext) -> list[RawItem]:
    """关 / 空输入 → 直通; 否则按 skip_source_types + 已有 popularity 跳过, 并发查 HN。"""
    emit(ctx.logger, "enrich_start", input_count=len(items),
    ...
    to_enrich = [it for it in items
                 if it.source_type.value not in skip_types
                 and not _has_popularity(it)]
    if not to_enrich:
        emit(ctx.logger, "enrich_done", enriched=0, skipped=len(items))
        return items
    sem = asyncio.Semaphore(max(1, config.concurrency))
    await asyncio.gather(*(_enrich_one(it, client, sem, ctx)
                           for it in to_enrich))
    enriched = sum(1 for it in to_enrich if "hn_points" in it.signals)
    emit(ctx.logger, "enrich_done", enriched=enriched,
         skipped=len(items) - len(to_enrich), queried=len(to_enrich))
    return items
