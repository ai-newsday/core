...
def compute_quality_weights(
    stats: list[SourceFeedbackStats], prior_weights: dict[str, float], config: FeedbackConfig
...
def feedback(
    ...
    无网络/LLM/落盘副作用。"""
    emit(ctx.logger, "feedback_start", run_id=ctx.run_id, event_count=len(events))
    if not events:
        emit(ctx.logger, "feedback_done", event_count=0, source_count=0, silent=True)
        return FeedbackResult(
            source_stats=[],
            quality_weights=dict(prior_weights),
            weight_diff={},
            event_count=0,
            source_count=0,
            is_silent=True,
    ...
    emit(ctx.logger, "weights_computed", source_count=len(stats), changed_count=changed)
    emit(
        ctx.logger, "feedback_done", event_count=len(events), source_count=len(stats), silent=False
    )
    return FeedbackResult(
        source_stats=stats,
