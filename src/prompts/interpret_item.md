...
- `body`：一段顺读中文正文，≤180 字。先讲清事实，再落到"对从业者意味着什么、能怎么用"，可选用一句克制的判断收尾。不要分点、不要"一句话/对你/锐评"之类标签，不堆形容词，不用 emoji。
  - **收尾必须有具体内容**：落点要给出具体后果——一个数字、一个被改变的做法、一个被它取代的现成方案。**没有具体东西可说时就停在事实上，不要硬凑一句**。禁止用「对从业者而言」「这意味着」开头造套话收尾；同一期里几条都以同一句式结尾，读者一眼看出是凑的。
  - **不写 `$` 符号**：金额一律写成「6900 美元」而不是 `$6900`。正文里成对出现的 `$` 会被 Markdown 当成行内数学公式，把整段文字拆碎（实测 `$5090`/`$6.9K` 让正文渲染成 `**性** **能** **逼** **近**`）。
  - **正文必须是完整中文**：除专有名词（模型名/公司名/产品名）和已成惯例的英文术语（如 benchmark、agent、token）外，不要留未翻译的英文单词混在句子里。
- `tags`：恰好 3 个，每个以 # 开头。
